...
2. The web app must expose a public landing at `/` that separates the acquisition story into distinct jobs: product promise, problem framing, operational flow, Brain decision logic, user outcomes, and final app entry. The operational flow must stay static and understandable, while any Brain explainer must remain a transparency/decision deep-dive rather than a separate AI product promise.
...
- Public landing preview numbers, when shown, must come from curated demo cents and shared money utilities, never from ad hoc floating-point math, live repositories, or implied user data.
