...
describe("LandingBrainHero", () => {
  it("displays the cinematic Brain explainer with the predictive engine copy", () => {
    ...
    expect(screen.getByText(/La stima serve a decidere stanotte, non a impressionare\./i)).toBeInTheDocument()
    expect(screen.getByRole("heading", { name: /Margine\./i })).toBeInTheDocument()
    expect(screen.getByText(/una nuova rata o un abbonamento entrano davvero nel quadro termico del mese/i)).toBeInTheDocument()
