...
import { calculatePortfolioProjections, PortfolioMetrics } from "../logic/multi-goal-orchestrator"
...
export function useGoalPortfolio({ globalProjectionInput }: UseGoalPortfolioProps) {
    ...
    const setRhythm = async (type: "baseline" | "balanced" | "aggressive", label: string, benefitCents: number) => {
        if (!portfolio) return
