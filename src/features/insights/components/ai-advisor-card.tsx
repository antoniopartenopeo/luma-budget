...
export function AIAdvisorCard() {
    ...
    return (
        ...
        >
            <Card className="relative overflow-hidden rounded-[2rem] border border-white/40 shadow-xl bg-white/40 backdrop-blur-3xl p-1">
                ...
                <CardHeader className="relative pb-2 z-10 px-6 pt-6">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            <div className="h-10 w-10 rounded-xl bg-white border border-white/50 flex items-center justify-center shadow-sm">
                            ...
                            <div>
                                <CardTitle className="text-xl font-bold tracking-tight text-slate-900 flex items-center gap-2">
                                    Luma AI
                                    <Badge variant="secondary" className="bg-white/60 text-slate-600 hover:bg-white border-white/40 text-[10px] px-1.5 h-5 shadow-sm">BETA</Badge>
                                </CardTitle>
                                <CardDescription className="text-slate-500 text-xs font-bold uppercase tracking-wider">Financial Intelligence</CardDescription>
                            </div>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent className="relative z-10 pt-6 px-6 pb-6 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                    ...
                    {forecast && (
                        <div className="relative group rounded-2xl bg-white/50 border border-white/40 hover:bg-white/70 transition-all duration-300 p-5 shadow-sm">
                            ...
                            <div className="space-y-1 relative">
                                <div className="text-3xl sm:text-4xl font-extrabold tracking-tighter text-slate-900">
                    ...
                    {subscriptions.length > 0 && (
                        <div className="rounded-2xl bg-white/50 border border-white/40 hover:bg-white/70 transition-all duration-300 p-5 shadow-sm flex flex-col justify-between">
                            <div className="space-y-3">
                                ...
                                <div className="flex items-baseline gap-2">
                                    <span className="text-3xl font-extrabold text-slate-900">{subscriptions.length}</span>
                                    <span className="text-sm text-slate-500">attivi questo mese</span>
                                </div>
                            </div>

                            <div className="mt-4 flex flex-wrap gap-2">
                                {subscriptions.slice(0, 3).map((sub, i) => (
                                    <div key={i} className="px-2 py-1 rounded-md bg-white border border-slate-100 text-xs font-medium text-slate-600 shadow-sm">
                                        Servizio #{i + 1}
                                    </div>
                                ))}
                    ...
                    {/* 3. Smart Tip */}
                    {tips.length > 0 && (
                        <div className="md:col-span-2 lg:col-span-1 rounded-2xl bg-gradient-to-br from-indigo-50/80 to-purple-50/80 border border-indigo-100/50 p-5 relative overflow-hidden shadow-sm">
                            <div className="flex items-center gap-1.5 text-sm font-bold text-indigo-600 mb-3 uppercase tracking-wider">
                            ...
                            <div className="mt-4 flex justify-end">
                                <Button variant="ghost" size="sm" className="h-8 text-xs text-indigo-600 hover:text-indigo-800 hover:bg-indigo-100/50 -mr-2">
