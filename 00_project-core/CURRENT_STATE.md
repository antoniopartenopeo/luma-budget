...
- Landing sustainable quota visual added (May 11): `/` now replaces both the comparison Q&A block and the Brain/curve scroll explainer with a poster-style quota surface that emphasizes the monthly sustainable amount, real-month calculation, income, recurring costs, and end-of-month balance.
...
- Public landing now encodes a six-stage acquisition story: product promise -> why the current month is hard to read -> how Numa turns existing movements into an answer -> sustainable monthly quota as the practical output -> what users gain -> final app entry.
- The public landing no longer renders a dedicated Brain/curve explainer; decision logic is represented by the sustainable quota visual.
...
- The quota visual must remain synchronized with motion governance, user-language copy, and the single-surface reduced-motion rule.
- Landing copy can drift back into repetition if `landing.json`, `content.ts`, `landing-page.tsx`, and quota/outcome tests are not reviewed together.
- Landing quota visuals can drift into fragile competitor claims if the “other apps show the past” framing becomes absolute or unsupported.
...
- Re-check landing docs, motion rules, support surfaces, quota/landing tests, and public-link tests whenever `/` changes its narrative order, public links, preview behaviors, or first-viewport trust copy
