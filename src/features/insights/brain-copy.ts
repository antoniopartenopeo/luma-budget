...
export const BRAIN_CONSISTENCY_LABEL = "Stabilità"
...
export function resolveBrainStage(snapshot: NeuralBrainSnapshot | null): StageState {
    if (!snapshot) {
        return {
            id: "dormant",
            label: "Non attivo",
            summary: "Il Brain è fermo. Avvialo per iniziare a leggere il tuo storico.",
    ...
    if (trainedSamples === 0) {
        return {
            ...
            summary: "Il Brain è stato avviato ma non ha ancora abbastanza dati per imparare.",
    ...
    if (trainedSamples < 36) {
        return {
            id: "imprinting",
            label: "Sta imparando",
            summary: "Il Brain sta iniziando a riconoscere le tue abitudini dallo storico disponibile.",
            badgeVariant: "secondary",
        }
    }
    ...
    return {
        id: "adapting",
        label: "Attivo",
            summary: "Il Brain ha già una buona base e continua ad aggiornarsi quando arrivano nuovi dati.",
        badgeVariant: "secondary",
    }
}
...
export function formatBrainEvolutionReason(result: BrainEvolutionResult): { title: string; detail: string; tone: EventTone } {
    if (result.reason === "trained") {
        return {
            ...
            detail: `Il Brain ha riletto ${result.sampleCount} dati e ha aggiornato stime e segnali. Stabilità attuale ${result.averageLoss.toFixed(4)}.`,
            tone: "positive",
        }
    }
    ...
    if (result.reason === "no-new-data") {
        return {
            ...
            detail: "Dall'ultima analisi non sono arrivati dati nuovi utili, quindi il Brain non aggiorna la stima.",
            tone: "neutral",
        }
    }
    ...
    if (result.reason === "insufficient-data") {
        return {
            ...
            detail: `Servono ancora più dati per ottenere letture stabili: mesi utili ${result.monthsAnalyzed}, dati letti ${result.sampleCount}.`,
            tone: "warning",
        }
    }
...
export function formatBrainTopbarAriaLabel(percent: number): string {
    return `Vai al Brain (${percent}% pronto)`
}
