...
function SheetContent({
  ...
  return (
    <SheetPortal>
      ...
      >
        {children}
        <SheetPrimitive.Close className={cn("absolute right-5 top-5 inline-flex h-9 w-9 items-center justify-center rounded-full text-foreground/70 transition-[background-color,color,opacity] duration-200 hover:text-foreground focus:ring-2 focus:ring-ring/40 focus:outline-hidden disabled:pointer-events-none", LIQUID_CAPSULE_CLASS, LIQUID_REFRACTION_CLASS)}>
...
function SheetFooter({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("relative z-10 mt-auto flex flex-col gap-2 border-t border-white/16 bg-white/18 px-6 py-5 backdrop-blur-sm sm:px-7 dark:border-white/10 dark:bg-white/[0.03]", className)}
      {...props}
    />
  )
}
