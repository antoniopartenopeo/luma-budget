...
describe("LandingPage", () => {
  it("renders the public acquisition story and keeps navigation constrained to the landing plus app entry", () => {
    ...
    expect(screen.getByRole("heading", { name: /Il tuo saldo è il passato\. Scopri il tuo futuro\./i })).toBeInTheDocument()
    expect(screen.getByText(/Prevedi il margine del mese in un istante\./i)).toBeInTheDocument()
    ...
    expect(screen.getAllByRole("link", { name: /Inizia gratis/i }).length).toBeGreaterThan(0)
